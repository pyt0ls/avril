...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (args.length < 2) {
      // Lê prefixo do arquivo
      const prefixos = JSON.parse(fs.readFileSync(path.resolve(__dirname, "../../database/prefixos.json")));
      const prefixo = prefixos[message.guild.id] || "k?";
      return message.channel.send(`❌ Uso incorreto. Use: \`${prefixo}addcargo <usuário> <cargo>\``);
    }
    ...
    if (role.position >= message.guild.members.me.roles.highest.position) {
      return message.channel.send("❌ Não posso adicionar esse cargo, ele está acima do meu cargo.");
    }
    ...
    if (user.roles.cache.has(role.id)) {
      return message.channel.send("⚠️ Este usuário já possui esse cargo.");
    }
    ...
    try {
      ...
      console.error(err);
      message.channel.send("❌ Erro ao tentar adicionar o cargo. Verifique permissões e hierarquia.");
    }
  },
};